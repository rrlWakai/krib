...
export default function Reports() {
  ...
  const totalBookings = reservationTrends.reduce((sum, d) => sum + d.total, 0)
        ...
        <div className="border border-[#ECECEC] rounded-lg bg-white p-5">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 font-body text-[12px] text-[#757575]">Monthly reservation activity for 2026</p>
            </div>
            <div className="flex items-center gap-1 rounded-md bg-[#f0f2f7] px-3 py-1.5">
              <TrendingUp size={13} className="text-[#0A1F44]" />
              <span className="font-body text-[12px] font-medium text-[#0A1F44]">+15%</span>
            </div>
          </div>
        ...
        <div className="border border-[#ECECEC] rounded-lg bg-white p-5">
          ...
          <div className="relative h-[180px] sm:h-[200px]">
            ...
            <div className="absolute inset-y-0 left-9 right-0 flex items-end">
              <svg viewBox={`0 0 ${occupancyData.length * 60} ${maxOccupancy}`} className="h-[140px] w-full sm:h-[160px]" preserveAspectRatio="none">
                <defs>
            ...
            <div key={villa.villaName} className="border border-[#ECECEC] rounded-lg bg-white p-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-0.5 font-display text-[22px] font-medium text-[#0A1F44]">{villa.averageStay} nights</p>
                </div>
              </div>
              <div className="mt-4">
                <div className="mb-1 flex items-center justify-between">
                  <span className="font-body text-[11px] text-[#757575]">Booking share</span>
                  <span className="font-body text-[11px] font-medium text-[#0A1F44]">
                    {((villa.totalBookings / villaPopularity.reduce((s, v) => s + v.totalBookings, 0)) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="h-1.5 overflow-hidden rounded-full bg-[#ECECEC]">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${(villa.totalBookings / villaPopularity.reduce((s, v) => s + v.totalBookings, 0)) * 100}%` }}
            ...
            {statusDistribution.map((item, i) => {
              ...
              return (
                <div key={item.status} className="flex items-center gap-3">
                  ...
                  <span className="w-28 font-body text-[13px] text-[#0A1F44]">{item.status}</span>
