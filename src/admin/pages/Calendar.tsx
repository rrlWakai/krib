...
const HISTORICAL_STATUSES: ReservationStatus[] = [
  "completed",
  "declined",
  "cancelled",
];
...
function overlapsDay(res: Reservation, dayKey: string): boolean {
  return (
    reservationDayKey(res.arrival_datetime) <= dayKey &&
    dayKey <= reservationDayKey(res.checkout_datetime)
  );
}
...
export default function Calendar() {
  ...
  const [currentYear, setCurrentYear] = useState(
    () => manilaParts(new Date()).year,
  );
  const [currentMonth, setCurrentMonth] = useState(
    () => manilaParts(new Date()).month - 1,
  );
  ...
  const [currentDayKey, setCurrentDayKey] = useState(() =>
    businessDayKey(new Date()),
  );
  const [villaFilter, setVillaFilter] = useState<string>("all");
  const [statusFilters, setStatusFilters] =
    useState<ReservationStatus[]>(ACTIVE_STATUSES);
  ...
  const [manualBookingVillaId, setManualBookingVillaId] = useState<
    string | undefined
  >(undefined);
      ...
      if (updated) {
        reservationsQuery.setData((prev) => {
          ...
          next[idx] = {
            ...prev[idx],
            ...updated,
            guest: prev[idx].guest,
            villa: prev[idx].villa,
          };
          return next;
        });
  ...
  const filteredReservations = useMemo(() => {
    let r = [...reservations];
    if (villaFilter !== "all")
      r = r.filter((res) => res.villa.slug === villaFilter);
  ...
  const occupancy = useMemo(() => {
    ...
    const arrivals = reservations.filter(
      (r) =>
        r.status === "approved" &&
        businessDayKey(r.arrival_datetime) === todayKey,
    );
    const departures = reservations.filter(
      (r) =>
        r.status === "approved" &&
        businessDayKey(r.checkout_datetime) === todayKey,
    );
    ...
    const upcomingCheckins = reservations.filter(
      (r) =>
        r.status === "approved" &&
        (businessDayKey(r.arrival_datetime) === todayKey ||
          businessDayKey(r.arrival_datetime) === tomorrowKey),
    );
  ...
  const getReservationsBlockingDate = useCallback(
    (dateKey: string) => {
    ...
    ? villas.filter(
        (villa) =>
          ...
          !selectedDateReservations.some(
            (reservation) => reservation.villa_id === villa.id,
          ),
      )
    : [];
  ...
  function openManualBooking() {
    ...
    setManualBookingVillaId(
      availableVillasForSelectedDate.length === 1
        ? availableVillasForSelectedDate[0].slug
        : undefined,
    );
  }
  ...
  if (reservationsQuery.error || villasQuery.error) {
    return (
      <div>
        ...
        <ErrorBlock
          message={
            reservationsQuery.error ?? villasQuery.error ?? "Unknown error"
          }
    ...
    <div>
      ...
      <div className="border border-[#ECECEC] rounded-lg bg-white overflow-hidden">
              ...
              <div className="flex flex-wrap items-center gap-2 px-5 py-3">
                ...
                {statusFilters.length > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1 rounded-md px-2.5 py-1 font-body text-[11px] text-[#757575] transition-colors hover:bg-[#f0f2f7]"
                  >
                    <X size={12} /> Clear
                  </button>
                )}
        ...
        <div className="flex flex-wrap items-center gap-4 border-b border-[#ECECEC] px-5 py-2">
          {ALL_STATUSES.map((s) => (
            <span
              key={s}
              className="flex items-center gap-1.5 font-body text-[11px] text-[#757575]"
            >
              <span
                className={cn(
                  "h-1.5 w-1.5 rounded-full",
                  STATUS_STYLE[s]?.dot ?? STATUS_STYLE.pending.dot,
                )}
              />
              {s.replace("_", " ")}
            </span>
        ...
        <div className="flex items-center justify-between px-5 pt-4">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full transition-colors hover:bg-[#f0f2f7]"
          >
          ...
          <h2 className="font-display text-[20px] font-medium text-[#0A1F44]">
            ...
            {view === "week" && (
              <>
                {formatManilaDateKey(weekKeys[0], {
                  month: "short",
                  day: "numeric",
                })}{" "}
                –{" "}
                {formatManilaDateKey(weekKeys[6], {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}
            ...
            {view === "day" &&
              formatManilaDateKey(currentDayKey, {
                weekday: "long",
                month: "long",
                day: "numeric",
                year: "numeric",
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#ECECEC] bg-[#FAFAFA] px-4 py-3">
              <div>
                <p className="font-body text-[13px] font-medium text-[#0A1F44]">
                  {formatManilaDateKey(selectedDateKey, {
                    weekday: "long",
                    month: "long",
                    day: "numeric",
                  })}
              ...
              <div className="flex items-center gap-2">
                {selectedDateReservations[0] && (
                  <button
                    type="button"
                    onClick={() =>
                      setSelectedReservation(selectedDateReservations[0])
                    }
      ...
      {view === "month" && (
        <div className="mt-6 md:hidden">
          <h3 className="mb-3 font-display text-[16px] font-medium text-[#0A1F44]">
            Upcoming This Month
          </h3>
            ...
            {filteredReservations
              ...
              .map((res) => (
                <button
                  ...
                  className="flex items-center gap-3 rounded-lg border border-[#ECECEC] bg-white p-3 text-left"
                >
                  <div className="flex w-10 flex-col items-center">
                    <span className="font-body text-[10px] text-[#757575]">
                      {formatManilaDateKey(
                        businessDayKey(res.arrival_datetime),
                        { month: "short" },
                      )}
                    </span>
                    <span className="font-body text-[14px] font-medium text-[#0A1F44]">
                      {manilaDayOfKey(businessDayKey(res.arrival_datetime))}
                    </span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-body text-[13px] font-medium text-[#0A1F44]">
                      {res.guest.full_name}
                    </p>
                    <p className="font-body text-[11px] text-[#757575]">
                      {res.villa.name}
    ...
    <>
      <div className="mb-3 grid grid-cols-7 gap-0">
        {WEEKDAYS.map((day) => (
          <div
            key={day}
            className="py-2 text-center font-body text-[11px] uppercase tracking-[0.08em] text-[#757575]"
          >
            {day}
          </div>
        ))}
        ...
        {dates.map((day, idx) => {
          if (day === null)
            return (
              <div
                key={`e-${idx}`}
                className="min-h-[90px] bg-white md:min-h-[120px]"
              />
            );
              ...
              <div className="flex flex-col gap-1">
                {dayRes.slice(0, 2).map((res) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={cn("h-0.5 w-full rounded-full mb-1", s.bar)}
                      />
                ...
                {dayRes.length > 2 && (
                  <span className="px-1 font-body text-[9px] text-[#757575]">
        ...
        {reservations.map((res, idx) => {
          ...
          return (
            ...
            >
              ...
              <p className="truncate font-body text-[11px] font-medium text-[#0A1F44] w-full">
                {res.guest.full_name}
              </p>
              <p className="truncate font-body text-[10px] text-[#757575] w-full">
                {res.villa.name}
              </p>
              <p className="truncate font-body text-[10px] text-[#757575] w-full">
                {formatManilaTime(res.arrival_datetime)}
...
function DayView({
  ...
  const arrivals = reservations.filter(
    (r) => reservationDayKey(r.arrival_datetime) === dayKey,
  );
  const departures = reservations.filter(
    (r) => reservationDayKey(r.checkout_datetime) === dayKey,
  );
  const inHouse = reservations.filter(
    (r) =>
      reservationDayKey(r.arrival_datetime) < dayKey &&
      reservationDayKey(r.checkout_datetime) > dayKey,
  );
  ...
  return (
    <div className="space-y-5">
      {arrivals.length > 0 && (
        <section>
          <h4 className="mb-2 font-body text-[11px] uppercase tracking-[0.08em] text-[#0A1F44]">
            Arrivals
          </h4>
      ...
      {departures.length > 0 && (
        <section>
          <h4 className="mb-2 font-body text-[11px] uppercase tracking-[0.08em] text-[#757575]">
            Departures
          </h4>
          <div className="flex flex-col gap-1.5">
            {departures.map((res) => (
