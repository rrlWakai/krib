...
export default function ProfilePage() {
  ...
  const [saving, setSaving] = useState<"profile" | "email" | "password" | null>(
    null,
  );
  ...
  useEffect(() => {
    setDisplayName(
      profileNameFrom(user?.user_metadata?.full_name, admin?.full_name),
    );
    setEmail(user?.email ?? "");
  ...
  async function chooseAvatar(file: File | null) {
    ...
    if (!isSupportedAvatarFile(file) || file.size > MAX_AVATAR_SIZE) {
      setFeedback({
        section: "profile",
        type: "error",
        message: "Choose a JPG, PNG, WebP, or AVIF image smaller than 5 MB.",
      });
      return;
    }
  ...
  async function uploadAvatar() {
    ...
    const extension = normalizeAvatarExtension(
      avatarFile.type || avatarFile.name,
      "jpg",
    );
  ...
  async function saveEmail(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (nextEmail === user?.email) {
      setFeedback({
        section: "email",
        type: "error",
        message: "Enter a different email address to update it.",
      });
      return;
    }
    ...
    setFeedback(
      error
        ...
        : {
            ...
            type: "success",
            message:
              "Confirmation email sent. Your email changes after you confirm it.",
          },
    );
  }
  ...
  const initials = profileNameFrom(
        ...
        <section className="border border-[#ECECEC] rounded-lg bg-white p-5">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body text-[12px] text-[#757575]">
                Changing your email requires confirmation from Supabase Auth.
              </p>
            </div>
          </div>
...
function Feedback({ notice }: { notice: Exclude<Notice, null> }) {
  return (
    <div
      className={`mb-4 flex items-center gap-2 rounded-lg border px-4 py-3 font-body text-[13px] ${notice.type === "success" ? "border-[#B9D7BF] bg-[#F0F7F1] text-[#2F6B3B]" : "border-red-200 bg-red-50 text-red-700"}`}
    >
      {notice.type === "success" && <Check size={14} />} {notice.message}
    </div>
  );
}
