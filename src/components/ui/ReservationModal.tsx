...
import { X, CalendarDays, Clock, Check, Send, Phone, Mail, MessageCircle, FileText } from 'lucide-react'
...
const modalPanelVariants = {
  ...
  exit: { opacity: 0, scale: 0.97, y: 12, transition: { duration: 0.25, ease: [0.22, 1, 0.36, 1] as const } },
}

export function ReservationModal({ isOpen, onClose, property }: ReservationModalProps) {
  ...
  const [contactMethod, setContactMethod] = useState<ContactMethod>('messenger')
  ...
  function validate(): boolean {
    ...
    else if (!/^[\d\s+\-()]{7,20}$/.test(phone)) errs.phone = 'Please enter a valid phone number'
    if (contactMethod === 'messenger' && !facebookName.trim()) errs.facebookName = 'Facebook name is required'
  ...
  const contactMethods: { value: ContactMethod; label: string; icon: typeof MessageCircle }[] = [
          ...
          >
              ...
              <>
                ...
                <div className="shrink-0 px-8 md:px-10 pt-12 pb-4">
                  ...
                  <p className="font-body text-body-md text-on-surface-variant leading-relaxed">
                    Fill in your details and we will review your reservation request.
                    You will not be charged yet.
                  </p>
                </div>

                {/* Scrollable Body */}
                <div className="flex-1 overflow-y-auto overscroll-contain min-h-0 px-8 md:px-10 pb-4" style={{ WebkitOverflowScrolling: 'touch' }}>
                  {/* Trip Details */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div className="p-4 bg-surface-container-low rounded-default">
                      ...
                      <p className="font-body text-body-md font-medium text-on-surface">{property.name}</p>
                    </div>
                    <div className="p-4 bg-surface-container-low rounded-default">
                      <label className="font-body text-label-caps text-on-surface-variant/60 uppercase tracking-widest text-[11px] block mb-1.5">
                        Check-in Date
                      ...
                      />
                      {errors.checkIn && <p className="font-body text-[12px] text-error mt-1">{errors.checkIn}</p>}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div className="p-4 bg-surface-container-low rounded-default">
                      ...
                      <p className="font-body text-body-md text-on-surface font-medium">
                        {checkOut || <span className="text-on-surface-variant/50">Auto-calculated</span>}
                      </p>
                    </div>
                    <div className="p-4 bg-surface-container-low rounded-default">
                      <label className="font-body text-label-caps text-on-surface-variant/60 uppercase tracking-widest text-[11px] block mb-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-body text-body-md text-primary font-medium">22-Hour Stay</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 mb-8">
                    <div className="flex justify-between items-center">
                      <span className="font-body text-body-md text-on-surface-variant">Villa Rate</span>
                      <span className="font-body text-body-md font-medium text-on-surface">{property.priceDetails.perNight}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-body text-body-md text-on-surface-variant">Party Fee</span>
                      <span className="font-body text-body-md font-medium text-on-surface">{property.hasPartyFee ? property.partyFeeLabel ?? '₱5,000' : '—'}</span>
                    </div>
                    <div className="border-t border-outline-variant/50 pt-3 flex justify-between items-center">
                      <span className="font-body text-body-md font-semibold text-on-surface">Total</span>
                      <span className="font-display text-headline-md font-medium text-primary">{property.priceDetails.perNight}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 mb-8">
                    <div>
                      ...
                      />
                      {errors.fullName && <p className="font-body text-[12px] text-error mt-1">{errors.fullName}</p>}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-default font-body text-body-md text-on-surface placeholder:text-on-surface-variant/40 focus:outline-none focus:border-primary transition-colors duration-200"
                        />
                        {errors.phone && <p className="font-body text-[12px] text-error mt-1">{errors.phone}</p>}
                      </div>
                      <div>
                        <label className="font-body text-label-caps text-on-surface-variant/60 uppercase tracking-widest text-[11px] block mb-1.5">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {contactMethods.map((m) => {
                          ...
                          return (
                            ...
                            >
                              <Icon size={16} />
                              <span className="font-body text-body-md text-sm">{m.label}</span>
                            </button>
                          )
                        })}
                      </div>
                    ...
                    <AnimatePresence>
                      {contactMethod === 'messenger' && (
                        ...
                        >
                          <div>
                            <label className="font-body text-label-caps text-on-surface-variant/60 uppercase tracking-widest text-[11px] block mb-1.5">
                              Facebook Name <span className="text-error">*</span>
                            </label>
                            <input
                              ref={facebookNameRef}
                              type="text"
                              ...
                              className="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-default font-body text-body-md text-on-surface placeholder:text-on-surface-variant/40 focus:outline-none focus:border-primary transition-colors duration-200"
                            />
                            {errors.facebookName && <p className="font-body text-[12px] text-error mt-1">{errors.facebookName}</p>}
                          </div>
                          <div>
                            <label className="font-body text-label-caps text-on-surface-variant/60 uppercase tracking-widest text-[11px] block mb-1.5">
                  ...
                  <div className="p-5 bg-surface-container-low rounded-default border border-outline-variant/40">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {errors.agreed && <p className="font-body text-[12px] text-error mt-1">{errors.agreed}</p>}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {step === 'submitting' && (
              <div className="flex-1 flex items-center justify-center p-12">
                <div className="text-center">
                  ...
                  <p className="font-body text-body-md text-on-surface-variant">Submitting your reservation request...</p>
                </div>
              </div>
            )}
            ...
            {step === 'success' && (
              <div className="flex-1 overflow-y-auto">
                <SuccessState onClose={onClose} propertyName={property.name} />
              </div>
            )}
          </div>
          </motion.div>
...
function SectionDivider({ icon: Icon, label }: { icon: typeof CalendarDays; label: string }) {
  return (
...
function SuccessState({ onClose, propertyName }: { onClose: () => void; propertyName: string }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        Thank you for choosing KRiB Beverly Place. We have received your reservation
        request for <strong className="text-on-surface">{propertyName}</strong>. Our team
        will review your selected date and contact you through your preferred contact
        method once your reservation has been reviewed.
      </motion.p>
